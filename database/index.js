...
const insertIntoPatientTable = (patient, callback) => {
  const sql = `INSERT INTO Patients (firstName, middleName, lastName, age, gender,location, maritalStatus,
               phoneNumber, email, insurance, id_Progress, id_Doctor, createdAt) VALUES ('${
                 patient.firstName
               }', '${patient.MiddleName}'
    ...
    patient.id_Progress
  }', '${patient.id_Doctor}', "${moment().format()}");`;
  dbConnection.query(sql, function(err, result) {
    if (err) {
...
const insertConsultations = (data, callback) => {
  ...
  const sql = `insert into Consultants (id_Doctors, id_targetDoctor, subject, description,createdAt)
                values("${data.doctorId}", "${data.targetDoctorId}", "${
    data.subject
  }", "${data.description}", "${moment().format()}")`;
  dbConnection.query(sql, function(err, result) {
    if (err) {
...
const selectConsultationOutbox = (doctorId, callback) => {
  ...
  dbConnection.query(sql, function(err, results) {
    if (err) {
      console.log(
        "Error during select consultions outbox for doctor  \n" + err
      );
      callback(err, null);
    } else {
      callback(null, results);
...
const selectOneConsultation = (consultId, doctorId, callback) => {
  ...
  dbConnection.query(sql, function(err, results) {
    if (err) {
      console.log(
        "Error during select one consultion outbox for doctor  \n" + err
      );
      callback(err, null);
    } else {
      callback(null, results);
...
const AddChiefComplaint = (data, callback) => {
  const sql = `INSERT INTO ChiefComplaint (caseId,title,description, createdAt) 
               VALUES ('${data.id}','${data.description}','${
    data.title
  }',"${moment().format()}") `;
  dbConnection.query(sql, function(err, results) {
    if (err) {
      console.log(
        "Error during update info  from ChiefComplaint Table \n" + err
      );
      callback(err, null);
    } else {
      callback(null, results);
...
const AddPhysicalExamination = (data, callback) => {
  const sql = `INSERT INTO PhysicalExamination (caseId,weight,height,bodyTemperature,headNotes,
                    middleBodyNotes,bottomBodyNotes,diabetes,BloodPressure, createdAt) VALUES ('${
                      data.id
                    }','${data.weight}',
                    '${data.height}','${data.bodyTemperature}','${
    data.headNotes
  }','${data.middleBodyNotes}',
                    '${data.bottomBodyNotes}','${data.diabetes}','${
    data.BloodPressure
  }', "${moment().format()}") `;
  dbConnection.query(sql, function(err, results) {
    if (err) {
      console.log(
        "Error during update info  from physicalExamination Table \n" + err
      );
      callback(err, null);
    } else {
      callback(null, results);
...
const AddMedicalPrescription = (data, callback) => {
  const sql = `INSERT INTO MedicalPrescription (caseId,name, daysInterval, times, createdAt)
   VALUES ('${data.id}','${data.name}','${data.daysInterval}','${
    data.times
  }', "${moment().format()}") `;
  dbConnection.query(sql, function(err, results) {
    if (err) {
      console.log(
        "Error during update info  from MedicalPrescription Table \n" + err
      );
      callback(err, null);
    } else {
      callback(null, results);
...
const updateDoctorInfo = (newData, callback) => {
  let sql = `update Doctors set firstName = "${newData.firstName}", lastName="${
    newData.lastName
  }", specialist = "${newData.specialty}" 
  ,phoneNumber= "${newData.phoneNumber}", bio="${newData.bio}" , gender= "${
    newData.gender
  }", birthDate = "${newData.bDate}", nationality = "${newData.nationality}",
  location = "${newData.location}", clinicName = "${
    newData.clinicName
  }", clinicNumber = "${newData.clinicNumber}" where id = ${newData.id};`;
...
const AddAppointment = (data, callback) => {
  const sql = `INSERT INTO Appointment (id_Doctors,id_Patients,notes,date) VALUES ('${
    data.id_Doctors
  }','${data.id_Patients}','${data.notes}','${data.date}') `;
  dbConnection.query(sql, function(err, results) {
    if (err) {
...
const AddnewCase = (data, callback) => {
  const sql = `INSERT INTO PatientCases (patientId,title,description,isOpen) VALUES ('${
    data.patientId
  }','${data.title}','${data.description}','${data.isOpen}') `;
  dbConnection.query(sql, function(err, results) {
    if (err) {
...
const UpdatePlanStep = (data, callback) => {
  const sql = `UPDATE PatientPlane SET step='${data.step}' WHERE id='${
    data.Id
  }' `;
  dbConnection.query(sql, function(err, results) {
    if (err) {
  ...
  UpdatePlanStep(data, function(err, result) {
    if (err) {
      ...
      dbConnection.query(sql, function(err, results) {
        if (err) {
          console.log(
            "Error during update info  from PatientCases close Table \n" + err
          );
          callback(err, null);
        } else {
          callback(null, results);
  ...
  UpdatePlanStep(data, function(err, result) {
    if (err) {
      ...
      dbConnection.query(sql, function(err, results) {
        if (err) {
          console.log(
            "Error during update info  from PatientCases open  Table \n" + err
          );
          callback(err, null);
        } else {
...
const AddPatientPlan = (data, callback) => {
  const sql = `INSERT INTO PatientPlane (caseId,PhysicalPlan,MedicalPlan,Conclusion,step, createdAt) 
                VALUES ('${data.CaseId}','${data.PhysicalPlan}','${
    data.MedicalPlan
  }','${data.Conclusion}', 0, "${moment().format()}");`;
...
const AddPatientHistory = (data, callback) => {
  const sql = `INSERT INTO MedicalHistory (caseId,heartDisease,joints,bloodPressure,diabetes,renalDisease,patientHistory,familyHistory, createdAt)
   VALUES ('${data.CaseId}','${data.heartDisease}','${data.joints}','${
    data.bloodPressure
  }','${data.diabetes}',
   '${data.renalDisease}','${data.patientHistory}','${
    data.familyHistory
  }', "${moment().format()}") `;
  dbConnection.query(sql, function(err, results) {
    if (err) {
...
const searchDoctor = (target, callback) => {
  const sql = `select * FROM Doctors WHERE firstName like "%${target}%" || phoneNumber like "%${target}%"`;
  dbConnection.query(sql, function(err, results) {
    if (err) {
      console.log("Error during search info  from Doctor Table \n" + err);
      callback(err, null);
    } else {
      callback(null, results);
    }
...
module.exports.searchDoctor = searchDoctor;
